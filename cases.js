...
module.exports = [
  ...
  /* 1) Pfote/Wunde/Schwellung */
  {
    ...
    match: (t, lang) => {
      ...
      return lang === 'en'
        ? (/(paw|pad|nail)/.test(s) && /(inflam|red|swoll|wound|pus|cut|crack)/.test(s))
        : (/(pfote|ballen|kralle)/.test(s) && /(entzünd|rot|schwell|wund|eiter|schnitt|riss)/.test(s));
    },
    start: (text, s, L) => {
      s.state.step = 1;
    ...
    step: (text, s) => {
      s.state.data.text = (s.state.data.text || '') + ' ' + text;
      if (s.state.step === 1) {
        s.state.step = 2;
        const t = s.state.data.text;
        const long = /(tage|woche|seit.*tag|seit.*woche)/i.test(t);
        const strong = /(gar nicht|kaum|stark|nicht belastet)/i.test(t);
        const foreignNo = /\bnein\b/i.test(t);
        const out =
      ...
      return "Kurzes Update: besser/schlechter?";
    },
  ...
  /* 3) Starke Blutung / Blutung unklar */
  {
    ...
    match: (t, lang) => {
      const s = t.toLowerCase();
      return lang === 'en'
        ? /(bleeding|bleeds a lot|spurting blood|severe cut)/.test(s)
        : /(starke blutung|blutet stark|spritzend|pulsierend|tiefer schnitt|platzwunde)/.test(s) || /(blutung|blutet|schnitt|platzwunde|offene wunde)/.test(s);
    },
    start: (text, s) => {
      s.state.name = null;
      return "⚠️ **Blutung – Sofortmaßnahmen:**\n1) **Druckverband** 5–10 Min. ohne Unterbrechung.\n2) Stelle leicht hochlagern.\n3) Ruhig + warm halten, nichts in die Wunde füllen.\n4) **Umgehend Tierarzt/Notdienst** aufsuchen.";
    },
  ...
  /* 4) Vergiftung */
  {
    ...
    match: (t, lang) => {
      const s = t.toLowerCase();
      return lang === 'en'
        ? /(poison|toxin|ate rat poison|chocolate|xylitol|ibuprofen|grapes|raisins)/.test(s)
        : /(vergift|gift|rattenköder|schokolade|xylit|ibuprofen|trauben|rosinen)/.test(s);
    },
    start: (text, s) => {
      s.state.name = null;
      return "⚠️ **Vergiftungsverdacht:**\n1) **Nichts** einflößen, **kein** Erbrechen erzwingen.\n2) Verpackung/Foto sichern.\n3) **Sofort** Tierarzt/Notdienst anrufen – Stoff, Menge, Zeit, Gewicht nennen.\n4) Taumeln/Krämpfe → **ohne Verzögerung losfahren**.";
    },
  ...
  /* 5) Aufgeblähter Bauch / GDV */
  {
    ...
    match: (t, lang) => {
      const s = t.toLowerCase();
      return lang === 'en'
        ? /(bloat|gdv|swollen belly|retches but nothing)/.test(s)
        : /(aufgeblähter bauch|magenumdrehung|magen\-torsion|würgen ohne erbrechen)/.test(s);
    },
    start: () => "⚠️ **Verdacht auf Magendrehung**: harter Bauch, Würgen ohne Erbrechen, Unruhe/Schmerz.\n→ **Sofort** Notdienst/Tierklinik – keine Zeit verlieren!",
  ...
  /* 7) Harnblockade / „kann nicht pinkeln“ */
  {
    ...
    match: (t, lang) => {
      const s = t.toLowerCase();
      return lang === 'en'
        ? /(can'?t pee|cannot pee|no urine|straining no urine)/.test(s)
        : /(kann nicht pinkeln|kein urin|strengt sich an und es kommt nichts)/.test(s) || /(urin|pinkelt|strengt sich an)/.test(s);
    },
    start: () => "⚠️ **Harnabfluss gestört** (Notfall möglich):\n1) Nicht warten – **sofort Tierarzt/Notdienst** (Gefahr durch Harnstau).\n2) Ruhig halten, Wasser anbieten – nicht forcieren.\n3) Bei Schmerzen/Unruhe → direkt losfahren.",
  ...
  /* 9) Durchfall */
  {
    ...
    step: (text, s) => {
      ...
      if (s.state.step === 1) {
        s.state.step = 2;
        const t = s.state.data.text;
        const long = /(48|zwei tage|2 tage|seit.*tagen)/i.test(t);
        const bloody = /\b(blut|blutig|schleim)\b/i.test(t);
        const leth = /\b(müde|apathisch|schwach)\b/i.test(t);
        const nodrink = /(trinkt nicht|kein wasser|trinkt kaum)/i.test(t);
      ...
      return "Sag Bescheid, falls Blut, Schwäche oder keine Besserung.";
    }
  },
  ...
  /* 10) Erbrechen */
  {
    ...
    step: (text, s) => {
      ...
      if (s.state.step === 1) {
        s.state.step = 2;
        const t = s.state.data.text;
        const many = /(3|drei|mehrfach|oft|häufig)/i.test(t);
        const blood = /\b(blut|rötlich)\b/i.test(t);
        const nowater = /(hält.*nicht|erbricht wasser|trinkt nicht)/i.test(t);
        const leth = /\b(müde|apathisch|schwach)\b/i.test(t);
      ...
      return "Gib ein Update, ob es besser wird.";
    }
  },
  ...
  /* 11) Humpeln/Lahmheit */
  {
    ...
    step: (text, s) => {
      ...
      if (s.state.step === 1) {
        s.state.step = 2;
        const t = s.state.data.text;
        const sinceDays = /(tage|seit.*tag|woche)/i.test(t);
        const noWeight = /(gar nicht|nicht belastet|trägt nicht)/i.test(t);
        const swelling = /(schwell|dick|heiß|warm)/i.test(t);
        const accident = /(unfall|sturz|zerrung|umgeknickt)/i.test(t);
        ...
        const out =
          ...
          `4) ${alarm ? 'Tierarzt innerhalb 24 h.' : 'Wenn keine Besserung → Tierarzt.'}`;
        s.state.name = null;
        return out;
      }
      return "Kurzes Update, bitte.";
    },
    photo: (s, L) => L.photoReceived
  },
  ...
  /* 12) Auge */
  {
    ...
    match: (t, lang) => {
      const s = t.toLowerCase();
      return lang === 'en'
        ? /(eye|ocular|red eye|squint|discharge)/.test(s)
    ...
    step: (text, s) => {
      s.state.data.text = (s.state.data.text || '') + ' ' + text;
      if (!s.state.step) s.state.step = 1;
      const t = s.state.data.text;
      const severe = /(stark|sehr|verletz|fremdkörper|trüb|blut)/i.test(t);
      const out =
  ...
  /* 13) Ohr */
  {
    ...
    match: (t, lang) => {
      const s = t.toLowerCase();
      return lang === 'en'
        ? /(ear|otitis|shaking head|scratching ear|ear discharge)/.test(s)
    ...
    step: (text, s) => {
      ...
      if (!s.state.step) s.state.step = 1;
      const t = s.state.data.text;
      const severe = /(stark|eitrig|geruch|sehr rot|schmerz)/i.test(t);
      const out =
  ...
  /* 14) Zecke/Stich/Allergie */
  {
    ...
    match: (t, lang) => {
      const s = t.toLowerCase();
      return lang === 'en'
        ? /(tick|bee sting|wasp sting|allergic reaction|hives)/.test(s)
    ...
    step: (text, s) => {
      ...
      if (!s.state.step) s.state.step = 1;
      const t = s.state.data.text;
      const face = /(gesicht|zunge|augenlid|maul)/i.test(t);
      const breath = /(atemnot|keucht|schlecht luft)/i.test(t);
      const out =
        `Nächste Schritte:\n1) Kühlen, Ruhe.\n2) ${face || breath ? 'Sofort Tierarzt/Notdienst.' : 'Beobachten; bei starker Schwellung/Schwäche → Tierarzt.'}\n3) Nach Zecke: Stelle täglich ansehen; Fieber/Trägheit → abklären.`;
      s.state.name = null;
      return out;
    },
    photo: (s, L) => L.photoReceived
  ...
  /* 15) Husten/Atemwege */
  {
    ...
    match: (t, lang) => {
      const s = t.toLowerCase();
      return lang === 'en'
        ? /(cough|kennel cough|trachea|honking|breath|breathing)/.test(s)
  ...
  /* 16) Appetitlosigkeit */
  {
    ...
    match: (t, lang) => {
      const s = t.toLowerCase();
      return lang === 'en'
        ? /(no appetite|not eating|refuses food|stopped eating)/.test(s)
    ...
    step: (text, s) => {
      ...
      if (!s.state.step) s.state.step = 1;
      const t = s.state.data.text;
      const long = /(tage|seit.*tag|woche)/i.test(t);
      const alarm = /(erbricht|durchfall|fieber|schmerz|apathisch)/i.test(t);
      const out =
  ...
  /* 17) Verstopfung */
  {
    ...
    match: (t, lang) => {
      const s = t.toLowerCase();
      return lang === 'en'
        ? /(constipation|hard stool|straining to poop)/.test(s)
        : /(verstopfung|harte kot|drückt ohne erfolg)/.test(s);
    },
    ...
    step: (text, s) => {
      ...
      const t = s.state.data.text;
      const long = /(tage|seit.*tag|woche)/i.test(t);
      const alarm = /(starke schmerzen|aufgebläht|erbricht)/i.test(t);
      const out =
  ...
  /* 18) Zahn/Zahnfleisch */
  {
    ...
    match: (t, lang) => {
      const s = t.toLowerCase();
      return lang === 'en'
        ? /(tooth|teeth|gum|broken tooth|tooth pain)/.test(s)
    ...
    step: (text, s) => {
      ...
      if (!s.state.step) s.state.step = 1;
      const t = s.state.data.text;
      const broken = /(abgebrochen|bruch|splitter)/i.test(t);
      const bleedSmell = /(blutet|geruch)/i.test(t);
      const out =
