// index.js — Seelenpfote (Cases ausgelagert), Notfälle sofort, State korrekt
...
function findCase(text, lang) {
  for (const c of CASES) if (c.match(text, lang)) return c;
  return null;
}
...
// ---- Router (fix: active case zuerst weiterführen; Notfälle sofort)
function replyFor(text, s) {
  ...
  // 3) Keiner aktiv → neuen starten, falls erkannt
  if (detected) {
